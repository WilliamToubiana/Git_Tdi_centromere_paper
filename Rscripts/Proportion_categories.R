...
setwd("/work/FAC/FBM/DEE/tschwand/asex_sinergia/wtoubian/chip/coverage/GW")

###Coverage sequence categories Genome-Wide

#TRs
TR_cenh3<-read.table("Tdi_cenh3_testes_1_TR_coverage_GW.txt",
                     header=FALSE)
TR_input<-read.table("Tdi_input_testes_1_TR_coverage_GW.txt",
                     header=FALSE)
...
#exons
exon_cenh3<-read.table("Tdi_cenh3_testes_1_exon_coverage_GW.txt",
                       header=FALSE)
exon_input<-read.table("Tdi_input_testes_1_exon_coverage_GW.txt",
                       header=FALSE)
...
#introns
intron_cenh3<-read.table("Tdi_cenh3_testes_1_intron_coverage_GW.txt",
                         header=FALSE)
intron_input<-read.table("Tdi_input_testes_1_intron_coverage_GW.txt",
                         header=FALSE)
...
#5-UTRs
UTR5_cenh3<-read.table("Tdi_cenh3_testes_1_5-UTR_coverage_GW.txt",
                       header=FALSE)
UTR5_input<-read.table("Tdi_input_testes_1_5-UTR_coverage_GW.txt",
                       header=FALSE)
...
#3-UTRs
UTR3_cenh3<-read.table("Tdi_cenh3_testes_1_3-UTR_coverage_GW.txt",
                       header=FALSE)
UTR3_input<-read.table("Tdi_input_testes_1_3-UTR_coverage_GW.txt",
                       header=FALSE)
...
#ncRNAs
ncRNA_cenh3<-read.table("Tdi_cenh3_testes_1_ncRNA_coverage_GW.txt",
                        header=FALSE)
ncRNA_input<-read.table("Tdi_input_testes_1_ncRNA_coverage_GW.txt",
                        header=FALSE)

ncRNA<-cbind(ncRNA_cenh3,ncRNA_input[4])
colnames(ncRNA)<-c("Scaffold_name", "start", "end","coverage_cenh3", "coverage_input")
ncRNA$genomic_feature<-"ncRNA"



#rRNAs
rRNA_cenh3<-read.table("Tdi_cenh3_testes_1_rRNA_coverage_GW.txt",
                       header=FALSE)
rRNA_input<-read.table("Tdi_input_testes_1_rRNA_coverage_GW.txt",
                       header=FALSE)

rRNA<-cbind(rRNA_cenh3,rRNA_input[4])
colnames(rRNA)<-c("Scaffold_name", "start", "end","coverage_cenh3", "coverage_input")
rRNA$genomic_feature<-"rRNA"



#tRNAs
tRNA_cenh3<-read.table("Tdi_cenh3_testes_1_tRNA_coverage_GW.txt",
                       header=FALSE)
tRNA_input<-read.table("Tdi_input_testes_1_tRNA_coverage_GW.txt",
                       header=FALSE)

tRNA<-cbind(tRNA_cenh3,tRNA_input[4])
colnames(tRNA)<-c("Scaffold_name", "start", "end","coverage_cenh3", "coverage_input")
tRNA$genomic_feature<-"tRNA"




#LINEs
LINE_cenh3<-read.table("Tdi_cenh3_testes_1_TE_LINE_coverage_GW.txt",
                       header=FALSE)
LINE_input<-read.table("Tdi_input_testes_1_TE_LINE_coverage_GW.txt",
                       header=FALSE)
...
#SINEs
SINE_cenh3<-read.table("Tdi_cenh3_testes_1_TE_SINE_coverage_GW.txt",
                       header=FALSE)
SINE_input<-read.table("Tdi_input_testes_1_TE_SINE_coverage_GW.txt",
                       header=FALSE)
...
#LTRs
LTR_cenh3<-read.table("Tdi_cenh3_testes_1_TE_LTR_coverage_GW.txt",
                      header=FALSE)
LTR_input<-read.table("Tdi_input_testes_1_TE_LTR_coverage_GW.txt",
                      header=FALSE)
...
#RCs
RC_cenh3<-read.table("Tdi_cenh3_testes_1_TE_RC_coverage_GW.txt",
                     header=FALSE)
RC_input<-read.table("Tdi_input_testes_1_TE_RC_coverage_GW.txt",
                     header=FALSE)
...
#DNAs
DNA_cenh3<-read.table("Tdi_cenh3_testes_1_TE_DNA_coverage_GW.txt",
                      header=FALSE)
DNA_input<-read.table("Tdi_input_testes_1_TE_DNA_coverage_GW.txt",
                      header=FALSE)
...
enriched<-subset(data, log2cenH3_norm >= 4) #only select sequence categories with log2(ChIP/input) >= 4; use of different thresholds for Supplementary figure 3
...
data3A<-aggregate(data2A$array~data2A$region_genomic_feature, FUN=sum)
data3A[c('region', 'genomic_feature')] <- str_split_fixed(data3A$`data2A$region_genomic_feature`, '-', 2)
...
ggplot(data4A, aes(x = region, y = perc, fill = genomic_feature)) +
  geom_col(color="black", size=0.1) +scale_x_discrete(limits = c("genome",  "enriched"))+
  scale_fill_manual(values = c("#E2D1F9", "#2F3C7E", "#FCF6F5", "#8A307F", "#79A7D3", "white","#6883BC", "#EA738D", "#DDC3A5" , "#F7C5CC", "#990011", "#101820", "#E0A96D"))+
  theme_classic()
...
data2<-rbind(do.call("rbind", list(rRNA_ann, tRNA_ann, ncRNA_ann, exon_ann, intron_ann, UTR5_ann, UTR3_ann, TR_ann[,c(1,2,3,5,6)], LINE_ann, SINE_ann, LTR_ann, RC_ann, DNA_ann, enriched[,c(1,2,3,6,11)])))
data2$array<-data2$end-data2$start
data2chmX<-subset(data2, Scaffold_name=="Tdi_LRv5a_scf3")
data2chmX$region_genomic_feature<-paste(data2chmX$region, data2chmX$genomic_feature, sep="-")
data3X<-aggregate(data2chmX$array~data2chmX$region_genomic_feature, FUN=sum)
data3X[c('region', 'genomic_feature')] <- str_split_fixed(data3X$`data2chmX$region_genomic_feature`, '-', 2)


data4X <- data3X %>%
  group_by(region) %>%                     # grouped by rep
  mutate(sum_rep=sum(`data2chmX$array`)) %>%          # sum of each rep
  group_by(region,genomic_feature) %>%            # grouped by DB
  summarise(perc=sum(`data2chmX$array`)/unique(sum_rep))


ggplot(data4X, aes(x = region, y = perc, fill = genomic_feature)) +
  geom_col(color="black", size=0.1) +scale_x_discrete(limits = c("genome",  "enriched"))+
  scale_fill_manual(values = c("#E2D1F9", "#2F3C7E", "#FCF6F5", "#8A307F", "#79A7D3", "white","#6883BC", "#EA738D", "#DDC3A5" , "#F7C5CC", "#990011", "#101820", "#E0A96D"))+
  theme_classic()
...
tableA<-xtabs(`data2A$array` ~ region+genomic_feature, data=data3A)
chisq.test(tableA)

tableX<-xtabs(`data2chmX$array` ~ region+genomic_feature, data=data3X)
chisq.test(tableX)
