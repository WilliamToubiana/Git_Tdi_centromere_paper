...
#Identification most abundant families
family_abundance<-data.frame(tapply(nonoverlapWind$array_length, nonoverlapWind$group, FUN=sum))
colnames(family_abundance)[1]<-"sum_array_length"
family_abundance$perc<-family_abundance$sum_width/sum(family_abundance$sum_width)


##Estimation of most represented motif length among the most abundant repeat families
library(DescTools)
Mode(group_all$motif_length[group_all$group =="group1"])
Mode(group_all$motif_length[group_all$group =="group2"])
Mode(group_all$motif_length[group_all$group =="group7"])
Mode(group_all$motif_length[group_all$group =="group13"])
Mode(group_all$motif_length[group_all$group =="group22"])
