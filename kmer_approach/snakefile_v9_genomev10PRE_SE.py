...
def calculate_enrich_stats(all_enrich_values_file, output_stats_file, num_mads):
    from snakemake.utils import sys
    import pandas as pd
...
rule prepare_more_input:
    ...
    output: "yf_seqdat/{dataacc}/raw/{dataacc}.fa.gz"
    shell:'''
    ln -s $(readlink -f {input}) {output}
    '''
...
rule generate_kmers:
    ...
    threads: 8
    shell: '''
    ./kmc_env/bin/kmc -k{wildcards.kmer_len} -m{params.mem} -sm -t{threads} -ci{wildcards.cival} -cs100000000 {input} {params.dbname} "$(dirname {input})"/tmp
    '''
...
rule dump_kmers:
    ...
    params:
        dbname = "{samplename}/kmer_dbs/{basename}_{readnum}/{kmer_len}/{cival}/kcounts"
    shell: '''./kmc_env/bin/kmc_tools transform {params.dbname} dump -s {output.dump_sort}
    '''
...
rule bbduk:
    input:
        in1 = "yf_seqdat/{dataacc}/raw/{dataacc}.fa.gz",
